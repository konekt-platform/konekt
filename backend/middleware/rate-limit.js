...
function checkRateLimit(key, limit, windowMinutes) {
  ...
  const entry = rateLimitStore.get(storeKey) || { count: 0, resetAt: Date.now() + (windowMinutes * 60 * 1000) };
...
export function rateLimitMiddleware(request, reply, done) {
  ...
  if (path === '/events' && request.method === 'POST' && userId) {
    ...
    if (!eventLimit.allowed) {
      return reply.code(429).send({
        error: 'Limite de criação de eventos atingido. Máximo de 10 eventos por hora.',
  ...
  if (path === '/posts' && request.method === 'POST' && userId) {
    ...
    if (!postLimit.allowed) {
      return reply.code(429).send({
        error: 'Limite de criação de posts atingido. Máximo de 2 posts por hora.',
