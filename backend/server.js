...
function buildAttendeesList(event, data) {
  ...
  const attendeesList = attendeeIds.map((userId) => {
...
function sanitizeEvent(event, data) {
  ...
  eventChat.forEach((msg) => {
    if (msg.authorId) {
      ...
      const user = data.users.find((u) => u.name === msg.author || u.username === msg.author);
      if (user) {
  ...
  const finalAttendeesList = finalAttendeeIds.map((userId) => {
    const user = data.users.find((u) => compareIds(u.id, userId));
    if (!user) return null;
    return {
      id: user.id,
...
await app.register(cors, {
  origin: (origin, cb) => {
    ...
    const isExplicit = allowed.includes(origin);
    const isTunnel = /^https:\/\/[a-z0-9-]+\.trycloudflare\.com$/.test(origin)
      || /^https:\/\/[a-z0-9-]+\.loca\.lt$/.test(origin);
    return cb(null, isExplicit || isTunnel);
  },
});
...
app.get('/search', async (request) => {
  ...
  if (type === 'all' || type === 'events') {
    ...
    if (requestingUser && requestingUser.blockedUserIds) {
      events = events.filter((e) => {
        ...
        return !requestingUser.blockedUserIds.some((blockedId) => compareIds(blockedId, e.creatorId));
      });
    }
  ...
  if (type === 'all' || type === 'users') {
    ...
    if (requestingUser && requestingUser.blockedUserIds) {
      users = users.filter((u) => !requestingUser.blockedUserIds.some((blockedId) => compareIds(blockedId, u.id)));
    }
...
app.post('/auth/login', async (request, reply) => {
  ...
  data.tokens.push({ token, userId: user.id, createdAt: now, lastActivity: now });
  writeData(data);
...
app.post('/auth/register', async (request, reply) => {
  ...
  if (!email || !password || !name || !birthDate) {
    return reply.code(400).send({ error: 'email, password, name e birthDate são obrigatórios' });
  }
  ...
  const token = crypto.randomBytes(24).toString('hex');
  const now = new Date().toISOString();
  data.tokens.push({ token, userId: newUser.id, createdAt: now, lastActivity: now });
  writeData(data);
...
app.post('/auth/google-mock', async (request, reply) => {
  ...
  if (!email || !name || !birthDate) {
    return reply.code(400).send({ error: 'email, name e birthDate são obrigatórios' });
  }
  if (!isValidBirthDate(birthDate)) {
    return reply.code(400).send({ error: 'Data de nascimento inválida' });
  }
...
app.post('/users/me/avatar', async (request, reply) => {
  ...
  try {
    ...
    if (!avatarFile) {
      return reply.code(400).send({ error: 'Nenhum arquivo de avatar enviado' });
    }

    // Validar tipo MIME
    const mimeType = avatarFile.mimetype || '';
    if (!ALLOWED_TYPES.includes(mimeType)) {
      return reply.code(400).send({ error: 'Formato de imagem não suportado. Use JPG, PNG ou WEBP' });
    }

    // Validar extensão
    let ext = path.extname(avatarFile.filename || '').toLowerCase();
    if (!ALLOWED_EXTENSIONS.includes(ext)) {
      return reply.code(400).send({ error: 'Extensão de arquivo não suportada' });
    }

    // Validar tamanho
    const buffer = await avatarFile.toBuffer();
    if (buffer.length > MAX_SIZE) {
      return reply.code(400).send({ error: 'Arquivo muito grande. Tamanho máximo: 5MB' });
    }
    ...
    try {
      ...
      if (sharpModule) {
        ...
        app.log.warn('Sharp não disponível, usando imagem original sem redimensionamento');
      }
    } catch (resizeError) {
      app.log.warn('Erro ao redimensionar imagem, usando original:', resizeError);
    ...
    return reply.code(200).send({ avatar: avatarUrl, user: sanitizeUser(user) });
  } catch (error) {
    app.log.error('Erro ao fazer upload de avatar:', error);
    return reply.code(500).send({ error: 'Erro ao processar upload de avatar' });
  }
});
...
app.put('/users/me/privacy', async (request, reply) => {
  ...
  const { profilePublic, showEmail, showBirthDate, showFollowers } = request.body || {};
...
app.get('/users', async (request) => {
  ...
  return sanitized.filter((user) => user.username?.toLowerCase().includes(query));
});
...
app.get('/users/:id', async (request, reply) => {
  ...
  if (requestingUser) {
    ...
    if (requestingUser.blockedUserIds.some((blockedId) => compareIds(blockedId, user.id)) ||
      user.blockedUserIds.some((blockedId) => compareIds(blockedId, requestingUser.id))) {
...
app.post('/users/:id/follow', async (request, reply) => {
  ...
  if (isFollowing) {
    ...
    target.followerIds = target.followerIds.filter((id) => !compareIds(id, me.id));
  } else {
...
app.post('/users/:id/block', async (request, reply) => {
  ...
  const targetId = request.params.id;
  if (compareIds(me.id, targetId)) {
    return reply.code(400).send({ error: 'Não é possível bloquear a si mesmo' });
  }
  const target = data.users.find((u) => compareIds(u.id, targetId));
  if (!target) return reply.code(404).send({ error: 'Usuário não encontrado' });
...
app.post('/users/:id/unblock', async (request, reply) => {
  ...
  me.blockedUserIds = me.blockedUserIds.filter((id) => !compareIds(id, targetId));
...
app.get('/users/me/friends', async (request, reply) => {
  ...
  const friends = me.followingIds
    .filter((followingId) => me.followerIds.some((followerId) => compareIds(followerId, followingId)))
    .map((friendId) => {
...
app.post('/events/:id/join', async (request, reply) => {
  ...
  if (!event.isRecurring) {
    const endsAt = event.endsAt ? new Date(event.endsAt) : new Date(`${event.date} ${event.time}`);
...
app.post('/events/:id/checkin', async (request, reply) => {
  ...
  event.attendeeIds = event.attendeeIds || [];
  if (!event.attendeeIds.some((uid) => compareIds(uid, user.id))) {
    return reply.code(400).send({ error: 'Você precisa participar do evento antes de fazer check-in' });
  }

  // Verificar se evento já começou (opcional - pode remover se quiser permitir check-in antes)
  const startsAt = event.startsAt ? new Date(event.startsAt) : new Date(`${event.date} ${event.time}`);
  if (!event.isRecurring && !Number.isNaN(startsAt.getTime()) && startsAt.getTime() > Date.now()) {
  ...
  try {
    ...
    if (!photoFile) {
      return reply.code(400).send({ error: 'Foto é obrigatória para check-in' });
    }
    ...
    const participation = data.eventParticipations[id].find((p) => compareIds(p.userId, user.id));
    if (participation) {
...
app.get('/users/me/participations', async (request, reply) => {
  ...
  const enriched = participations.map((participation) => {
    const event = data.events.find((e) => compareIds(e.id, participation.eventId));
    return {
      ...participation,
  ...
  if (dateFrom) {
    enriched = enriched.filter((p) => p.event && new Date(p.event.date) >= new Date(dateFrom));
  }
  if (dateTo) {
    enriched = enriched.filter((p) => p.event && new Date(p.event.date) <= new Date(dateTo));
  }
...
app.put('/events/:id/series', async (request, reply) => {
  ...
  if (event.creatorId && !compareIds(event.creatorId, user.id)) {
    return reply.code(403).send({ error: 'Apenas o criador pode editar a série' });
  }
  ...
  const seriesEvents = data.events.filter((e) => {
    ...
    return e.isRecurring && compareIds(e.creatorId, event.creatorId) && e.type === event.type;
  });

  // Aplicar atualizações apenas a eventos futuros
  const now = Date.now();
  seriesEvents.forEach((seriesEvent) => {
    const eventDate = new Date(seriesEvent.startsAt || `${seriesEvent.date} ${seriesEvent.time}`);
    if (eventDate.getTime() > now) {
      ...
      if (updates.maxAttendees !== undefined) seriesEvent.maxAttendees = updates.maxAttendees;
      if (updates.visibility) seriesEvent.visibility = updates.visibility;
      if (updates.requiresApproval !== undefined) seriesEvent.requiresApproval = updates.requiresApproval;
  ...
  writeData(data);
  return { ok: true, message: 'Série atualizada com sucesso', updatedCount: seriesEvents.length };
});
...
app.delete('/events/:id/occurrence', async (request, reply) => {
  ...
  if (event.creatorId && !compareIds(event.creatorId, user.id)) {
    return reply.code(403).send({ error: 'Apenas o criador pode cancelar ocorrências' });
  }
  ...
  if (!event.recurringSeries.cancelledOccurrences.some((occId) => compareIds(occId, event.id))) {
...
app.post('/events/:id/series/join', async (request, reply) => {
  ...
  const seriesEvents = data.events.filter((e) => {
    if (e.recurringSeries?.seriesId) {
      ...
      // Fallback
      if (!e.isRecurring || !compareIds(e.creatorId, event.creatorId) || e.type !== event.type) {
        return false;
      }
    }
  ...
  seriesEvents.forEach((seriesEvent) => {
    ...
    if (seriesEvent.requiresApproval) {
      if (!seriesEvent.pendingRequestIds.some((uid) => compareIds(uid, user.id))) {
...
app.post('/events/:id/approve', async (request, reply) => {
  ...
  if (event.pendingRequestIds.some((uid) => compareIds(uid, userId))) {
    event.pendingRequestIds = event.pendingRequestIds.filter((uid) => !compareIds(uid, userId));
...
app.post('/events', async (request, reply) => {
  ...
  const existing = requestedId !== undefined && requestedId !== null
  ...
  const newEvent = {
    ...event,
    id: (requestedId !== undefined && requestedId !== null) ? requestedId : generateId(), // Era: Date.now()
...
app.put('/events/:id', async (request, reply) => {
  ...
  if (clientVersion !== undefined && clientVersion !== event.version) {
    return reply.code(409).send({
      ...
      message: 'O evento foi modificado por outra pessoa. Recarregue a página e tente novamente.',
...
app.get('/events/:id/expenses', async (request, reply) => {
  ...
  const expenses = (data.expenses || [])
    .filter(e => compareIds(e.eventId, eventId));
...
app.put('/events/:id/expenses/:expenseId', async (request, reply) => {
  ...
  const expenseIndex = (data.expenses || []).findIndex(e => compareIds(e.id, expenseId));

  if (expenseIndex === -1) return reply.code(404).send({ error: 'Despesa não encontrada' });
...
app.put('/events/:id/expenses/:expenseId/participants/:participantId', async (request, reply) => {
  const data = readData();
  ensureDataCollections(data);
  const user = getAuthUser(request, data);
  if (!user) return reply.code(401).send({ error: 'Não autenticado' });
  ...
  const participantIndex = (expense.participants || []).findIndex(p => compareIds(p.userId, participantId));
  if (participantIndex === -1) return reply.code(404).send({ error: 'Participante não encontrado na despesa' });

  if (isSelf && status === 'paid_waiting_confirmation') {
    expense.participants[participantIndex].status = 'paid_waiting_confirmation';
  } else if (isCreator) {
    if (status === 'paid' || status === 'pending') {
      expense.participants[participantIndex].status = status;
    } else {
      return reply.code(400).send({ error: 'Status inválido para o criador' });
    }
  } else {
  ...
  expense.participants[participantIndex].paid = (expense.participants[participantIndex].status === 'paid');
...
app.delete('/events/:id/expenses/:expenseId', async (request, reply) => {
  ...
  data.expenses = (data.expenses || []).filter(e => !compareIds(e.id, expenseId));
...
app.post('/posts/:id/like', async (request, reply) => {
  ...
  if (isLiked) {
    post.likedByIds = post.likedByIds.filter((uid) => !compareIds(uid, user.id));
  } else {
...
app.put('/notifications/:id/read', async (request, reply) => {
  ...
  if (!notif) return reply.code(404).send({ error: 'Notificação não encontrada' });
  notif.unread = false;
  writeData(data);
  return { ok: true };
});
...
function cleanupTokens() {
  try {
    ...
    if (removedCount > 0) {
      app.log.info(`Limpeza de tokens: ${removedCount} tokens expirados removidos`);
    }
  } catch (error) {
...
function cleanupUploads() {
  try {
    ...
    if (removedCount > 0) {
      app.log.info(`Limpeza de uploads: ${removedCount} arquivos órfãos removidos`);
    }
  } catch (error) {
...
function cleanupBackups() {
  try {
    ...
    if (removedCount > 0) {
      app.log.info(`Limpeza de backups: ${removedCount} backups antigos removidos`);
    }
  } catch (error) {
...
app.log.info(`  - Limpeza de tokens: a cada ${TOKEN_CLEANUP_INTERVAL / (60 * 60 * 1000)} horas`);
app.log.info(`  - Limpeza de uploads: a cada ${UPLOAD_CLEANUP_INTERVAL / (60 * 60 * 1000)} horas`);
