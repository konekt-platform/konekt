...
export function EventFullCard({
  ...
  const { data: remoteEvent, isError: isEventError } = useGetEvent(
    event.id,
    event,
  );
  ...
  const bannedKey = authUser
    ? `konekt_event_banned_${authUser.id}_${currentEvent.id}`
    : `konekt_event_banned_${currentEvent.id}`;
  ...
  const participationKey = authUser
    ? `konekt_event_participation_${authUser.id}`
    : "konekt_event_participation";
  const expensesKey = authUser
    ? `konekt_event_expenses_${authUser.id}_${currentEvent.id}`
    : `konekt_event_expenses_${currentEvent.id}`;
  const chatKey = authUser
    ? `konekt_event_chat_${authUser.id}_${currentEvent.id}`
    : `konekt_event_chat_${currentEvent.id}`;
  const mediaKey = authUser
    ? `konekt_event_media_${authUser.id}_${currentEvent.id}`
    : `konekt_event_media_${currentEvent.id}`;
  const favoriteKey = authUser
    ? `konekt_favorite_events_${authUser.id}`
    : `konekt_favorite_events`;
  const completedEventsKey = authUser
    ? `konekt_completed_events_${authUser.id}`
    : `konekt_completed_events`;

  const canManageExpenses =
    isParticipating || isStoredParticipant || isEventCreator;
  const canShowGroup =
    !currentEvent.requiresApproval || canManageExpenses || requestSent;
  ...
  const calculateParticipants = useCallback(() => {
    ...
    if (isEventCreator && authUser) {
      ...
      const creatorCheckInMsg = chatMessages.find(
        (m) => m.photoUrl && m.authorId === authUser.id,
      );
    ...
    attendeesList.forEach((attendee) => {
      ...
      const checkInMsg = chatMessages.find(
        (m) => m.photoUrl && m.authorId === attendee.id,
      );
      ...
      participantsMap.set(attendee.id, {
        ...
        status:
          getParticipantStatus(attendee.id) === "presente" ||
          hasCheckedInLocally
            ? "presente"
            : "pendente",
        isCreator: false,
    ...
    attendeeIds.forEach((userId) => {
      ...
      if (user) {
        // Busca foto de check-in
        const checkInMsg = chatMessages.find(
          (m) => m.photoUrl && m.authorId === userId,
        );
        ...
        participantsMap.set(userId, {
          ...
          avatar: user.avatar || "",
          status:
            getParticipantStatus(userId) === "presente" || hasCheckedInLocally
              ? "presente"
              : "pendente",
    ...
    if (
      canManageExpenses &&
      authUser &&
      !participantsMap.has(authUser.id) &&
      !bannedUserIds.has(authUser.id)
    ) {
      // Busca foto de check-in
      const checkInMsg = chatMessages.find(
        (m) => m.photoUrl && m.authorId === authUser.id,
      ...
      participantsMap.set(authUser.id, {
        ...
        avatar: authUser.avatar || "",
        status:
          getParticipantStatus(authUser.id) === "presente" || hasCheckedIn
            ? "presente"
            : "pendente",
    ...
    chatMessages.forEach((msg) => {
      if (!msg.authorId) {
        ...
        if (
          !participantsMap.has(msg.authorId) &&
          !bannedUserIds.has(msg.authorId)
        ) {
          const user = allUsers.find((u) => u.id === msg.authorId);
          if (user) {
            // Busca foto de check-in
            const checkInMsg = chatMessages.find(
              (m) => m.photoUrl && m.authorId === msg.authorId,
            );
    ...
    hasCheckedIn,
    mediaItems,
  ]); // Added mediaItems to dependencies
  ...
  useEffect(() => {
    ...
    const userCheckedIn = chatMessages.some(
      (msg) =>
        msg.photoUrl &&
        (msg.authorId === authUser.id ||
          msg.author === (authUser.name || authUser.username)),
    );
    ...
    if (userCheckedIn) {
      try {
        ...
        if (!completedEvents.includes(currentEvent.id)) {
          localStorage.setItem(
            completedEventsKey,
            JSON.stringify([...completedEvents, currentEvent.id]),
          );
          // Dispara evento para atualizar o perfil
          window.dispatchEvent(
            new CustomEvent("konekt:event-completed", {
              detail: currentEvent.id,
            }),
          );
        }
      } catch {
        ...
        if (result.event) {
            ...
            (prev: Event | undefined) =>
              prev
                ? {
                    ...
                    pendingRequestIds: (prev.pendingRequestIds || []).filter(
                      (id) => id !== authUser.id,
                    ),
                  }
                : prev,
          ...
          try {
            ...
            if (!ids.includes(currentEvent.id)) {
              localStorage.setItem(
                participationKey,
                JSON.stringify([...ids, currentEvent.id]),
              );
            }
          ...
          if (events) {
            ...
            if (updatedEvent) {
              queryClient.invalidateQueries({
                queryKey: ["event", currentEvent.id],
              });
            }
          }
        }, 500);
  ...
  const visibilityGlow = getVisibilityGlow(
                        ...
                        .then((result) => {
                          setChatMessages((prev) => {
                            const newMessage: ChatMessage = {
                              ...
                              author:
                                authUser?.name || authUser?.username || "Você",
          ...
          <TabsContent value="detalhes" className="flex-1 overflow-y-auto">
                    ...
                    <div className="space-y-3 mt-4">
                      {allExpensesToList.map((expense) => {
                        ...
                        const amountPerPerson =
                          splitCount > 0 ? expense.amount / splitCount : 0;
                            ...
                            <div className="space-y-1 pt-2 border-t border-border/50">
                              {expense.participants.map((participant) => {
                                ...
                                const isPending =
                                  ...
                                  (status as string) === "none";
                                const isWaiting =
                                  (status as string) ===
                                  "paid_waiting_confirmation";
                                const isPaid =
                                  (status as string) === "paid" ||
                                    ...
                                    <div className="flex items-center gap-2">
                                      ...
                                      {isCurrentUser && isPending && (
                                        <button
                                          onClick={async () => {
                                            try {
                                              await updateExpenseParticipantStatusRequest(
                                                currentEvent.id,
                                                expense.id,
                                                participant.id,
                                                "pending",
            ...
            {!isEventCreator && (
              <button
                        ...
                        if (authUser) {
                          try {
                            ...
                            localStorage.setItem(
                              participationKey,
                              JSON.stringify(
                                ids.filter((id) => id !== currentEvent.id),
                              ),
                            );
                          } catch {
                            // ignore storage errors
                ...
                disabled={
                  participateLoading || requestSent || isPastNonRecurring
                }
                className={`w-full text-white py-3 rounded-xl border-none cursor-pointer font-semibold text-[14px] transition-all hover:scale-[1.02] active:scale-100 disabled:opacity-70 flex items-center justify-center gap-2 mt-4 ${
                  participateSuccess ? "bg-green-500" : ""
                }`}
                style={
                  participateSuccess
                    ? {}
            ...
            {isActive && isParticipating && (
              <button
                ...
                className={`mt-3 w-full rounded-xl border border-border px-4 py-2 text-sm font-semibold transition-colors flex items-center justify-center gap-2 ${
                  hasCheckedIn
              ...
              onClick={() => {
                ...
                updateFavoritesRequest(next)
                  .then((result) => {
                    ...
                    setIsFavorite(result.eventIds.includes(currentEvent.id));
                    window.dispatchEvent(
                      new CustomEvent("konekt:favorites-updated", {
                        detail: result.eventIds,
                      }),
                    );
                  })
                  .catch(() => {
              ...
              <>
                {!canManageExpenses ? (
                  <div className="rounded-lg border border-dashed border-border bg-background p-3 text-xs text-muted-foreground">
                    Entre no evento para acessar o grupo e conversar com os
                    participantes.
                  </div>
                ) : (
                  <>
                      ...
                      {chatMessages.length === 0 ? (
                        ...
                        chatMessages.map((msg) => {
                          ...
                          const userAvatar =
                            allUsers.find(
                              (u) =>
                                u.id === msg.authorId || u.name === msg.author,
                            )?.avatar ||
                            authUser?.avatar ||
                            "";
                          ...
                          return (
                            ...
                            >
                              ...
                              <div
                                className={`flex flex-col gap-1 ${isCurrentUser ? "items-end" : "items-start"} max-w-[75%]`}
                              >
                                <div className="flex items-center gap-2">
                        ...
                        <input
                          ...
                          onChange={(e) => {
                            ...
                            reader.onload = () => {
                              ...
                              postEventChatRequest(currentEvent.id, {
                                photoUrl,
          ...
          <TabsContent value="participantes" className="flex-1 overflow-y-auto">
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                <p className="text-sm font-semibold text-foreground">
                  {participantsWithStatus.length}{" "}
                  {participantsWithStatus.length === 1
                    ? "participante"
                    : "participantes"}
                </p>
                      ...
                      {isEventCreator && !participant.isCreator && (
                        <button
                          ...
                          onClick={() => {
                            if (
                              confirm(
                                `Tem certeza que deseja remover ${participant.name} do evento? Ele não poderá mais participar.`,
                              )
                            ) {
          ...
          {isEventCreator && (
            <TabsContent value="editar" className="flex-1 overflow-y-auto">
              <div className="space-y-4">
                ...
                {isActive && (
                  <p className="text-xs text-muted-foreground">
                    Não é possível alterar data e hora após o evento ter
                    começado.
                  </p>
                )}
          ...
          <div className="w-full max-w-md rounded-2xl bg-background p-4">
            ...
            <div className="mt-3 space-y-3">
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  ...
                  <button
                    ...
                    onClick={() => {
                      if (
                        selectedUserIds.size === participantsWithStatus.length
                      ) {
                        setSelectedUserIds(new Set());
                      } else {
                        setSelectedUserIds(
                          new Set(participantsWithStatus.map((p) => p.id)),
                        );
                      }
                    }}
              ...
              <button
                ...
                disabled={
                  !expenseTitle || !expenseAmount || selectedUserIds.size === 0
                }
