...
export const toggleFollowRequest = async (
  userId: number,
): Promise<{ me: User; user: User; isFollowing: boolean }> => {
  return apiFetch<{ me: User; user: User; isFollowing: boolean }>(
    `/users/${userId}/follow`,
    {
      method: "POST",
      body: JSON.stringify({}),
...
export const getFavoritesRequest = async (): Promise<{
  eventIds: number[];
}> => {
  return apiFetch<{ eventIds: number[] }>("/users/me/favorites");
};

export const updateFavoritesRequest = async (
  eventIds: number[],
): Promise<{ eventIds: number[] }> => {
  return apiFetch<{ eventIds: number[] }>("/users/me/favorites", {
    method: "PUT",
...
export const uploadAvatarRequest = async (
  ...
  if (!res.ok) {
    const errorData = await res
      .json()
      .catch(() => ({ error: "Falha ao enviar avatar" }));
...
export const updatePrivacySettingsRequest = async (
...
export const acceptFriendRequest = async (
  userId: number,
): Promise<{ me: User; user: User }> => {
  try {
    return await apiFetch<{ me: User; user: User }>(
